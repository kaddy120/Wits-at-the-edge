const trackButton = document.getElementById('track-Me')
...
for (let i = 0; i < items.length; i++) {
  items[i].children[3].firstChild.addEventListener('click', async (e) => {
    ...
    try {
      const response = await fetch(`/meeting/meetingFinished/${items[i].children[3].lastChild.value}/${new Date()}`, { method: 'get' })
